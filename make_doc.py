...
# ===== 4 =====
heading('4. 日曜の廃棄・土日の発注見直し')
body('原案（小泉店長）：日曜日の廃棄、土日の発注の見直し。', bold=True)
body('【ブラッシュアップ・追記】')
bullet('曜日別の発注カーブを作る。曜日別・時間帯別の販売実績で発注基準値を分ける（住宅地寄りなら土日の買われ方が平日と別物になりやすい）。')
bullet('日曜の廃棄が多い主因は土曜夜～日曜の発注過多。土曜最終便・日曜朝便の数量を実績ベースで再設定する。')
bullet('日曜を絞りすぎると月曜朝が品薄になる。両端をセットで管理する。')

# ===== 5 追加提案 =====
heading('5. 全体への追加提案（実行マネジメント）')
...
r = note.add_run('※「日曜の廃棄・土日の発注見直し」は原案メモが途中までのため、'
                 '全文の確定後にさらに具体化します。')
set_jp_font(r, size=9, color=GREEN)
